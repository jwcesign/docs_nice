...
Referring to [Create ACK cluster with Terraform](https://github.com/cloudpilot-ai/examples/tree/main/clusters/ack-ondemand-terway) to create a cluster easily.
...
export ALIBABA_CLOUD_REGION=<your_cluster_region>    #config your region
export ALIBABACLOUD_AK=<alibaba_cloud_access_key>    #config ak
export ALIBABACLOUD_SK=<alibaba_cloud_secret_key>    #config sk
...
After the deployment is scaled down, you will see the new node are deleted:
```bash
$ kubectl get nodes
NAME                       STATUS   ROLES    AGE   VERSION
cn-shenzhen.172.16.1.58    Ready    <none>   22m   v1.31.1-aliyun.1
