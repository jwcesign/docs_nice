...
1. **Controller**: TThe controller runs every 30 seconds and evicts pods from a workload if multiple pods are scheduled on the same instance type. This behavior is enabled when the owning Deployment or StatefulSet has the label `workload.cloudpilot.ai/diversity-instancetype=true`.
