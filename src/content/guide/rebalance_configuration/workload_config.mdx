...
This option specifies whether a workload is compatible with **spot scheduling**, which allows workloads to run on spot instances that are often cheaper but may be terminated by the cloud provider with little notice. This configuration takes effect only if **Rebalanceable** is enabled.
...
3. Set **mysql** to be **Spot Friendly**, allowing it to be scheduled on spot instances as needed.
